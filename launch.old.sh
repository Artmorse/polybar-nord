...
if type "xrandr"; then
	...
	if [ $((len_monitors)) -eq 2 ]; then
		...
		MONITOR=$monitor_1 polybar -c $HOME/.config/polybar/bars-config little-nord-top &
		...
		MONITOR=$monitor_2 polybar -c $HOME/.config/polybar/bars-config little-nord-top &
		echo "The second top bar has been launched on the embedded monitor."
		MONITOR=$monitor_3 polybar -c $HOME/.config/polybar/bars-config little-nord-top &
